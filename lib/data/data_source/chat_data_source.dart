...
class ChatDataSource {
  ...
  Future<List<Thread>> getChatList({int page=0}) async {
    ...
    try {
      var response = await clientDio.get(NetworkPath.THREAD.rawValue+'?page=$page');
      ...
      if (response.statusCode == 200 &&
          response.data != null && response.data["data"]!=null && response.data["data"].length > 0) {
 ...
 // get message model list
  Future<List<MessageModel>> getMessageList({int? threadId=0}) async {
    ...
    try {
      var response = await clientDio.get(NetworkPath.MESSAGESLIST.rawValue+'?thread_id=$threadId');
      ...
      dio.FormData data = await dio.FormData.fromMap({
        ...
        if (photoController.image1!=null)
          "files[]":[await dio.MultipartFile.fromFile(
              photoController.image1!.path,
              filename: photoController.image1!.path.split('/').last,
              contentType: MediaType("image", "jpeg")
          )]
      });
      dio.Options options = await dio.Options(
        method: "POST",
        headers: {
  ...
  Future createThread({String thread_name="",String body="",files}) async {
    try {
      dio.FormData data = await dio.FormData.fromMap({
